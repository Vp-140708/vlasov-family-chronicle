...
const FamilyNode = ({ data }: { data: { member: FamilyMember } }) => {
  ...
  return (
    <div className={`p-4 rounded-xl border-t-4 shadow-2xl bg-white w-64 text-center border-stone-200 transition-all hover:scale-105 ${
...
const Tree = () => {
  ...
  return (
    <div className="w-full h-[calc(100vh-64px)] bg-[#f5f2ed]">
      <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} onNodeClick={(_, node) => node.data?.member && setSelectedMember(node.data.member)} fitView minZoom={0.01}>
